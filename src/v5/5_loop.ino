...
#define STOP_FINAL 8
#define STOP_QUALI 9
#define STRAIGHTEN_FOR_PARKING 10

int CASE = PID;

// -----Angles-----
#define CORRECTION_ANGLE 50
#define AVOIDANCE_ANGLE 55
#define TURNAROUND_ANGLE 50
#define PARKING_GYRO_OFFSET 1.25
double follow_cube_angle = 0;

// -----Distances-----
#define CORNER_DISTANCE_FINAL 10
#define CORNER_DISTANCE_QUALI 38 // maybe 40 if the battery is overcharged
#define CORNER_DISTANCE_PARKING 46
#define AFTER_CUBE_DISTANCE 12
...
void pass_cube(int cube_last) {
  ...
  move_cm_gyro(AFTER_CUBE_DISTANCE, MOTOR_SPEED, start_angle - cube_last * (AVOIDANCE_ANGLE + angle_addition));
...
void execute(String cmd) {
  ...
  if (FINAL) { // if we're in the final round
    ...
    if (CASE == STRAIGHTEN_FOR_PARKING && cmd[0] == 'W') {
      CASE = FIND_PARKING;
    ...
    if (CASE == POSITION_BEFORE_FIND_PARKING && cmd[0] == 'W') { // if we're positioning ourselves close to the wall and we're in its proximity
      CASE = STRAIGHTEN_FOR_PARKING; // start searching for the parking lot
      return;
    }

    if (CASE == PARK && (cmd[0] == 'W' && cmd[1] == 'P')) { // if we're positioning ourselves close to the wall and we're in its proximity
      move_cm_gyro(8, PARKING_SPEED, current_angle_gyro - turn_direction * 90); // position ourselves closer
    ...
    if (CASE != FIND_PARKING && CASE != POSITION_BEFORE_FIND_PARKING && CASE != PARK) {
      if ((cmd[0] == 'R' || cmd[0] == 'G') && millis() - last_cube_time >= PASS_CUBE_DELAY) { // if we're in the proximity of a cube
        ...
        if (turns == 8 && !turned) // if we have are in the position to turn around, the cube is red and we haven't turned around already
  ...
  if (msg) { // a turn was detected
    ...
    if (millis() - last_rotate > delay_walls) { // if we can make a turn
      ...
      else if (abs(current_angle_gyro - gx) < 10) { // if we're straight
...
void loop_function() {
  ...
  switch(CASE) {
    ...
    case PID: {
      ...
      if (millis() - last_rotate > FIRST_STOP_DELAY && turns >= 12 && !completed_run) { // if we did 3 runs of the round
        if (FINAL) {
    ...
    case FOLLOW_CUBE: {
      if (millis() - last_rotate > FIRST_STOP_DELAY && turns >= 12 && !completed_run) { // if we did 3 runs of the obstacle round, we need to stop and search for the parking
    ...
    case FIND_PARKING: {
      ...
      double err = current_angle_gyro + PARKING_GYRO_OFFSET - gx;
      pid_error_gyro = (err) * kp_gyro + (pid_error_gyro - pid_last_error_gyro) * kd_gyro;
      pid_last_error_gyro = pid_error_gyro;
      move_servo(pid_error_gyro);
      move_motor(MOTOR_SPEED);
    ...
    case POSITION_FOR_PARK: {
      ...
      move_cm_gyro(5, PARKING_SPEED, current_angle_gyro);
